...
public class ForumController implements Initializable, ScreenSwitcher {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		sendButton.setOnAction(event -> {
			...
			Collections.reverse(listViewContent);
			listViewContent.add(new Comment(activeUser.getNickname(), timeLabel.getText(), titleInput.getText(),
					textInput.getText()));
			Collections.reverse(listViewContent);
	...
	private void addComment() {
		try {
			...
			stmt.executeUpdate("insert into comments (id_user, datetime, title, comment_text) values ("
					+ activeUser.getUser_id() + ",'" + timeLabel.getText() + "' , '" + titleInput.getText() + "' , '"
					+ textInput.getText() + "');");
	...
	private void loadComments() {
		...
		try {
			...
			ResultSet rset = stmt.executeQuery(
					"select u.nickname, c.datetime, c.title, c.comment_text from comments c join users u on u.id=c.id_user");
			while (rset.next()) {
				comment = new Comment(rset.getString("nickname"), rset.getString("datetime"), rset.getString("title"),
